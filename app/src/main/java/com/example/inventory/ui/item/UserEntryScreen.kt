...
fun UserInputForm(
    ...
    val courses = listOf("1A", "1B", "2A", "2B", "3A", "3B")
    var expanded by remember { mutableStateOf(false) }  // Cambia a remember si no necesitas guardarlo en el estado
    ...
    Column(
        ...
        OutlinedTextField(
            ...
            onValueChange = { onValueChange(userDetails.copy(surname = it)) },
            label = { Text(stringResource(R.string.user_surname_req)) },
            colors = OutlinedTextFieldDefaults.colors(
                focusedContainerColor = MaterialTheme.colorScheme.secondaryContainer,
                unfocusedContainerColor = MaterialTheme.colorScheme.secondaryContainer,
                disabledContainerColor = MaterialTheme.colorScheme.secondaryContainer,
        ...
        ExposedDropdownMenuBox(
            expanded = expanded,
            onExpandedChange = { expanded = !expanded } // Cambiar el estado de expandido al hacer clic
        ) {
            OutlinedTextField(
                value = userDetails.curse,
                onValueChange = {},  // No se necesita cambiar el valor aquí
                label = { Text(stringResource(R.string.user_curse_req)) },
                colors = OutlinedTextFieldDefaults.colors(
                    focusedContainerColor = MaterialTheme.colorScheme.secondaryContainer,
                    unfocusedContainerColor = MaterialTheme.colorScheme.secondaryContainer,
                    disabledContainerColor = MaterialTheme.colorScheme.secondaryContainer,
            ...
            ExposedDropdownMenu(
                ...
                courses.forEach { course ->
                    DropdownMenuItem(
                        ...
                        onClick = {
                            onValueChange(userDetails.copy(curse = course))
                            expanded = false
                        }
                    )
                }
...
private fun UserEntryScreenPreview() {
    InventoryTheme {
        UserEntryBody(userUiState = UserUiState(
            UserDetails(
                name = "User Name", surname = "User Surname", curse = "2A"
            )
